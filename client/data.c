...
void do_send(GtkWidget *widget, t_test *t)
{
    ...
    if(message[0] == '\\' && message[1])
    {
        ...
        gtk_label_set_text(GTK_LABEL(t->statusLabel), "Unknown command, type \\h for help");
        gtk_entry_set_text(GTK_ENTRY(t->sendEntry), "");
        return;
    }
...
void *watcher_thread(void *t)
{
    ...
    while(1)
    {
        ...
        add_list_entry(timebuf, author, body, k != 'h', (t_test*)t);
        free(author);
        free(body);
    }
    return 0;
...
void on_togglebuttonState_toggled(GtkWidget *widget, t_test *t) {
    ...
    if (togglebutton_state && !t->togglebutton2State) {
        gtk_button_set_label(GTK_BUTTON(t->togglebutton), "Switch on");
        t->togglebutton1State = true;
    }
    else if (togglebutton_state && t->togglebutton2State) {
        gtk_button_set_label(GTK_BUTTON(t->togglebutton), "Включить");
        t->togglebutton1State = true;
    }
    else if (!togglebutton_state && t->togglebutton2State) {
        gtk_button_set_label(GTK_BUTTON(t->togglebutton), "Выключить");
        t->togglebutton1State = false;
    }
    else if (!togglebutton_state && !t->togglebutton2State) {
        gtk_button_set_label(GTK_BUTTON(t->togglebutton), "Switch off");
        t->togglebutton1State = false;
    }
...
void on_togglebuttonState_toggled_2(GtkWidget *widget, t_test *t) {
	...
	if (togglebutton_state) {
        ...
        gtk_label_set_text(GTK_LABEL(t->chatLabel), "Введите \\h для команд чата");
        ...
        gtk_label_set_text(GTK_LABEL(t->chatLabel), "Type \\h for help");
...
void init_chat_window(char *login, t_test *t)
{
    ...
    GtkBuilder *builder = gtk_builder_new_from_file("chat.glade");
    ...
    // GtkWidget *stackSwitcher = GTK_WIDGET(gtk_builder_get_object(builder, "stackSwitcher"));
	// t->stackSwitcher = stackSwitcher;
    GtkWidget *chatLabel = GTK_WIDGET(gtk_builder_get_object(builder, "chatLabel"));
    t->chatLabel = chatLabel;
    // t->sendEntry = sendEntry;
